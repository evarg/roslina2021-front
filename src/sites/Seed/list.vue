...
export default {
  ...
  data() {
    return {
      fields: [
        ...
        { key: "variant_name", label: "Odmiana" },
  ...
  methods: {
    ...
    showItem(id) {
      this.$router.push("seeds/" + id);
    },
    editItem(id) {
      this.$router.push("seeds/" + id + "/edit");
    },
    newItem() {
      this.$router.push("seeds/add");
    },
    deleteItem(id) {
      this.$confirm(
        ...
        this.$http
          .delete(this.$store.state.ApiURL + "seeds/" + id)
          .then(() => {
            this.fetchItems();
          });
      });
